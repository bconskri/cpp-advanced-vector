...
// "Магическое" число, используемое для отслеживания живости объекта
    inline const uint32_t DEFAULT_COOKIE9 = 0xdeadbeef;

    struct TestObj9 {
        TestObj9() = default;
        TestObj9(const TestObj9& other) = default;
        TestObj9& operator=(const TestObj9& other) = default;
        TestObj9(TestObj9&& other) = default;
        TestObj9& operator=(TestObj9&& other) = default;
        ~TestObj9() {
            cookie = 0;
        }
        [[nodiscard]] bool IsAlive() const noexcept {
            return cookie == DEFAULT_COOKIE9;
        }
        uint32_t cookie = DEFAULT_COOKIE9;
    };
    ...
    struct Obj9 {
        Obj9() {
            ...
            //std::cout << "num_default_constructed: " << num_default_constructed << std::endl;
        }

        explicit Obj9(int id)
                : id(id)  //
        {
            ++num_constructed_with_id;
            //std::cout << "num_constructed_with_id: " << num_constructed_with_id << std::endl;
        }

        Obj9(int id, std::string name)
                : id(id)
                , name(std::move(name))  //
        {
            ++num_constructed_with_id_and_name;
            //std::cout << "num_constructed_with_id_and_name: " << num_constructed_with_id_and_name << std::endl;
        }

        Obj9(const Obj9& other)
                : id(other.id)  //
        {
        ...
        Obj9(Obj9&& other) noexcept
                : id(other.id)  //
        {
            ++num_moved;
            //std::cout << "num_moved: " << num_moved << std::endl;
        }

        Obj9& operator=(const Obj9& other) {
            if (this != &other) {
                id = other.id;
                name = other.name;
                ++num_assigned;
                //std::cout << "num_assigned: " << num_assigned << std::endl;
            }
            return *this;
        }

        Obj9& operator=(Obj9&& other) noexcept {
            id = other.id;
            name = std::move(other.name);
            ++num_move_assigned;
            //std::cout << "num_move_assigned: " << num_move_assigned << std::endl;
            return *this;
        }

        ~Obj9() {
            ++num_destroyed;
            //std::cout << "num_destroyed: " << num_destroyed << std::endl;
            id = 0;
        }

        static int GetAliveObjectCount() {
            std::cout << "num_default_constructed: " << num_default_constructed << " "
                    << "num_copied: " << num_copied << " "
                    << "num_moved: " << num_moved << " "
                    << "num_constructed_with_id: " << num_constructed_with_id << " "
                    << "num_constructed_with_id_and_name: " << num_constructed_with_id_and_name << " "
                    << "num_destroyed: " << num_destroyed << " "
                    << "sum: " << (num_default_constructed + num_copied + num_moved + num_constructed_with_id
...
void Test12_1() {
    ...
    }
    {
        Vector<Obj9> v;
        v.Reserve(SIZE);
        assert(Obj9::GetAliveObjectCount() == 0);
    }
    {
        Vector<Obj9> v(SIZE);
        assert(Obj9::GetAliveObjectCount() == SIZE);
        const int old_copy_count = Obj9::num_copied;
        const int old_move_count = Obj9::num_moved;
        v.Reserve(SIZE * 2);
        assert(Obj9::GetAliveObjectCount() == SIZE);
        assert(Obj9::num_copied == old_copy_count);
        assert(Obj9::num_moved == old_move_count + static_cast<int>(SIZE));
    }
    assert(Obj9::GetAliveObjectCount() == 0);
}
...
void Test12_2() {
    ...
    Obj9::ResetCounters();
    {
        Obj9::default_construction_throw_countdown = SIZE / 2;
        try {
            Vector<Obj9> v(SIZE);
        ...
        assert(Obj9::num_default_constructed == SIZE / 2 - 1);
        assert(Obj9::GetAliveObjectCount() == 0);
    }
    Obj9::ResetCounters();
    {
        ...
        try {
            ...
            Vector<Obj9> v_copy(v);
            assert(false && "Exception is expected");
        } catch (const std::runtime_error&) {
            assert(Obj9::num_copied == SIZE / 2);
        } catch (...) {
            // Unexpected error
            assert(false && "Unexpected exception");
        }
...
void Test12_3() {
    ...
    }
    {
        Obj9::ResetCounters();
        {
            Vector<Obj9> v(MEDIUM_SIZE);
            v[MEDIUM_SIZE / 2].id = ID;
            assert(Obj9::num_default_constructed == MEDIUM_SIZE);
            Vector<Obj9> moved_from_v(std::move(v));
            assert(moved_from_v.Size() == MEDIUM_SIZE);
            assert(moved_from_v[MEDIUM_SIZE / 2].id == ID);
        }
        assert(Obj9::GetAliveObjectCount() == 0);

        assert(Obj9::num_moved == 0);
        assert(Obj9::num_copied == 0);
        assert(Obj9::num_default_constructed == MEDIUM_SIZE);
    }
    {
        Obj9::ResetCounters();
        Vector<Obj9> v_medium(MEDIUM_SIZE);
        v_medium[MEDIUM_SIZE / 2].id = ID;
        Vector<Obj9> v_large(LARGE_SIZE);
        v_large = v_medium;
        assert(v_large.Size() == MEDIUM_SIZE);
        assert(v_large.Capacity() == LARGE_SIZE);
        assert(v_large[MEDIUM_SIZE / 2].id == ID);
        assert(Obj9::GetAliveObjectCount() == MEDIUM_SIZE + MEDIUM_SIZE);
    }
    {
        Obj9::ResetCounters();
        Vector<Obj9> v(MEDIUM_SIZE);
        {
            ...
            assert(Obj9::GetAliveObjectCount() == LARGE_SIZE + LARGE_SIZE);
        }
        assert(Obj9::GetAliveObjectCount() == LARGE_SIZE);
    }
    assert(Obj9::GetAliveObjectCount() == 0);
    {
        Obj9::ResetCounters();
        ...
        Vector<Obj9> v_small(MEDIUM_SIZE / 2);
        v_small.Reserve(MEDIUM_SIZE + 1);
        const size_t num_copies = Obj9::num_copied;
        v_small = v;
        assert(v_small.Size() == v.Size());
        assert(v_small.Capacity() == MEDIUM_SIZE + 1);
        v_small[MEDIUM_SIZE - 1].id = ID;
        assert(Obj9::num_copied - num_copies == MEDIUM_SIZE - (MEDIUM_SIZE / 2));
    }
}
...
void Test12_4() {
    ...
    const size_t SIZE = 100'500;
    {
        ...
        assert(v.Size() == SIZE);
        assert(v.Capacity() == SIZE);
        assert(Obj9::num_default_constructed == SIZE);
    }
    assert(Obj9::GetAliveObjectCount() == 0);

    {
        ...
        assert(v.Capacity() == SIZE);
        assert(Obj9::num_destroyed == SIZE - NEW_SIZE);
    }
    assert(Obj9::GetAliveObjectCount() == 0);
    {
        Obj9::ResetCounters();
        ...
        assert(v[SIZE].id == ID);
        assert(Obj9::num_default_constructed == SIZE);
        assert(Obj9::num_copied == 1);
        assert(Obj9::num_constructed_with_id == 1);
        assert(Obj9::num_moved == SIZE);
    }
    assert(Obj9::GetAliveObjectCount() == 0);
    {
        Obj9::ResetCounters();
...
void Test12_5() {
    ...
    using namespace std::literals;
    {
        ...
        assert(Obj9::num_constructed_with_id_and_name == 1);
        assert(Obj9::GetAliveObjectCount() == 1);
    }
    assert(Obj9::GetAliveObjectCount() == 0);
    {
        Vector<TestObj9> v(1);
...
void Test12_6() {
    ...
    }
    {
        ...
        Vector<Obj9> v{SIZE};
        Obj9 obj{1};
        Vector<Obj9>::iterator pos = v.Insert(v.cbegin() + 1, obj);
        assert(v.Size() == SIZE + 1);
        assert(v.Capacity() == SIZE * 2);
        assert(&*pos == &v[1]);
        ...
        assert(Obj9::num_copied == 1);
        assert(Obj9::num_default_constructed == SIZE);
        assert(Obj9::GetAliveObjectCount() == SIZE + 2);
    }
    {
        Obj9::ResetCounters();
        Vector<Obj9> v;
        auto* pos = v.Emplace(v.end(), Obj9{1});
        assert(v.Size() == 1);
        assert(v.Capacity() >= v.Size());
        assert(&*pos == &v[0]);
        assert(Obj9::num_moved == 1);
        assert(Obj9::num_constructed_with_id == 1);
        assert(Obj9::num_copied == 0);
        assert(Obj9::num_assigned == 0);
        assert(Obj9::num_move_assigned == 0);
        assert(Obj9::GetAliveObjectCount() == 1);
    }
    {
        Obj9::ResetCounters();
    ...
    }
    {
        Obj9::ResetCounters();
        Vector<Obj9> v{SIZE};
        Vector<Obj9>::iterator pos = v.Insert(v.cbegin() + 1, Obj9{1});
        assert(v.Size() == SIZE + 1);
        assert(v.Capacity() == SIZE * 2);
        assert(&*pos == &v[1]);
        assert(v[1].id == 1);
        assert(Obj9::num_copied == 0);
        assert(Obj9::num_default_constructed == SIZE);
        assert(Obj9::GetAliveObjectCount() == SIZE + 1);
    }
    {
        Vector<TestObj9> v{SIZE};
        v.Insert(v.cbegin() + 2, v[0]);
        assert(std::all_of(v.begin(), v.end(), [](const TestObj9& Obj9) {
            return Obj9.IsAlive();
        }));
    }
    {
    ...
    }
    {
        ...
        Vector<Obj9> v{SIZE};
        v.Reserve(SIZE * 2);
        const int old_num_moved = Obj9::num_moved;
        assert(v.Capacity() == SIZE * 2);
        ...
        assert(Obj9::num_default_constructed == SIZE);
        assert(Obj9::num_constructed_with_id_and_name == 1);
        assert(Obj9::num_moved == old_num_moved + 1);
        assert(Obj9::num_move_assigned == SIZE - 3);
        assert(Obj9::num_assigned == 0);
    }
    {
        Obj9::ResetCounters();
        ...
        assert(Obj9::num_copied == 0);
        assert(Obj9::num_assigned == 0);
        assert(Obj9::num_move_assigned == SIZE - 2);
        assert(Obj9::num_moved == 0);
        assert(Obj9::GetAliveObjectCount() == SIZE - 1);
    }
}
